...
def batch_resistance_distances_adaptive(G, pairs):
    """
    ...
    # Strategy 1: Small component - compute all pairwise
    if n_nodes < 20:
        ...
        for source, sink in pairs:
            key1 = (source, sink)
            key2 = (sink, source)
            if key1 in all_resistances:
                resistance = all_resistances[key1]
            elif key2 in all_resistances:
                resistance = all_resistances[key2]
            else:
                resistance = None
            results.append(resistance if (resistance is not None and resistance > 0) else None)
        
        return results
    
    # Strategy 2: Large component with many pairs
    if n_pairs > 0.3 * n_nodes * (n_nodes - 1) / 2:
        all_resistances = nx.resistance_distance(G, weight='conductance', invert_weight=False)
    ...
    for source, sink in sampled_pairs:
        ...
        sampled_resistances.append(resistance if resistance > 0 else None)
    
    # Build full results array
    # For sampled: use actual values
    # For non-sampled: use mean of sampled values (representative estimate)
    valid_samples = [r for r in sampled_resistances if r is not None]
    mean_resistance = sum(valid_samples) / len(valid_samples) if valid_samples else None
    
    results = []
    sampled_set = set(sampled_indices)
    sampled_dict = dict(zip(sampled_indices, sampled_resistances))
    
    for i in range(n_pairs):
        if i in sampled_set:
            results.append(sampled_dict[i])
        else:
            results.append(mean_resistance) # Use mean as proxy for unsampled
...
def process_sample(sample_udp: pd.Series):
    ...
    for pathway, G in PATHWAY_GRAPHS.items():
        ...
        for component, corridor_pairs in corridors_by_component.items():
            ...
            resistances = batch_resistance_distances_adaptive(subgraph, corridor_pairs)
            
            # Process only non-None results
            for (source, sink), resistance in zip(corridor_pairs, resistances):
                if resistance is not None and resistance > 0:
                    #flow = 1.0 / resistance # We are not calculating current I=V/R.
                    flow = resistance
                    if is_inhibitory(G.nodes[sink].get("interaction_type", "")):
                        flow = -flow
                    total_flow += flow
        
        activities[pathway] = float(total_flow/(len(resistances) if len(resistance)>0 else 1))
