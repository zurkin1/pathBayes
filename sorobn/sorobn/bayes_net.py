...
class CDTAccessor:
    ...
    def __init__(self, series: pd.Series):
        self.series = series
        # Initialize a default RNG for cases where none is provided
        self.rng = np.random.default_rng()
    
        """Sample a row at random.
        
        The `sample` method of a Series is very slow. Additionally, it is not designed to be used
        repetitively and requires O(n) steps every time it is called. 
        
        ORIGINAL: Used Cython implementation of Vose's alias method (O(n) build, O(1) query)
        REPLACEMENT: Using numpy.random.choice (O(n) per sample, but simpler and cross-platform)
        
        Args:
            rng: Optional numpy random generator. If provided by BayesNet, use it for 
                 consistent seeding across the network. Otherwise use instance rng.
        """
    def sample(self, rng=None):
            probs = self.series.values
            probs = probs / probs.sum()
            
            if rng is None:
                # Use numpy
                sample_idx = self.rng.choice(len(self.series), p=probs)
            else:
                # BayesNet passes random.Random(), use numpy.random.choice instead
                sample_idx = np.random.choice(len(self.series), p=probs)
            
            return self.series.index[sample_idx]
        ...
        while True:
            ...
            for node in self.nodes:
                ...
                if node in self.parents:
                    ...
                    if condition in  P.cdt.series.index:
                        P_eff = P.cdt[condition]
                    else:
                        # Identify which parents are True.
                        true_parents = [p for p, val in zip(self.parents[node], condition) if val]
                        if not true_parents:
                            # All False → use baseline if available
                            if (False,) * len(self.parents[node]) in P.cdt:
                                P_eff = P.cdt[(False,) * len(self.parents[node])]
                            else:
                                # Fallback uniform prior
                                P_eff = pd.Series({False: 0.5, True: 0.5})
                        else:
                            # Collect conditional distributions for each single active parent
                            probs_true = []
                            for p in true_parents:
                                sub_cond = tuple(t if q == p else False for q, t in zip(self.parents[node], condition))
                                if sub_cond in  P.cdt.series.index:
                                    probs_true.append(P.cdt[sub_cond].get(True, 0.5))
                            # Combine via noisy-OR if any valid entries
                            if probs_true:
                                p_true = 1 - np.prod([1 - p for p in probs_true])
                                P_eff = pd.Series({False: 1 - p_true, True: p_true})
                            else:
                                P_eff = pd.Series({False: 0.5, True: 0.5})
