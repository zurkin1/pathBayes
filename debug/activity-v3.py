import pandas as pd
import numpy as np
from metrics import gaussian_scaling
from config import *
...
def process_sample(sample_udp):
    ...
    for pathway, interactions in pathway_interactions.items():
        #pathway_activity = process_pathway(sample_udp, pathway, interactions)
...
'''
def process_pathway(gene_expression, pathway, interactions):
    ...
    for interaction_idx, interaction in enumerate(interactions):
        ...
        for gene in interaction[0]:
            if gene in gene_expression.index:
                interaction_activity += gene_expression[gene]

        # Calculate output activity.
        output_activity = 0
        for gene in interaction[2]:
            if gene in gene_expression.index:
                output_activity += gene_expression[gene]
    ...
    return pathway_activity / max(1,interactions_counter) #Return overall pathway activity.
'''
